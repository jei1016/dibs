...
pub enum Change {
    ...
    /// Change a column's auto-generated status (add/remove sequence or identity).
    ...
    pub fn to_sql(&self, table_name: &str) -> String {
        ...
        match self {
            ...
            Change::AlterColumnAutoGenerated { name, to, .. } => {
                if *to {
                    // Adding auto-generation: create sequence and set default
                    ...
                    // Removing auto-generation: drop the default (sequence remains for safety)
                    format!(
                        "ALTER TABLE {} ALTER COLUMN {} DROP DEFAULT;",
                        qt,
                        quote_ident(name)
...
/// Compare two columns and emit changes for any property differences.
///
/// Uses exhaustive destructuring to ensure all fields are considered.
/// If a new field is added to Column, this code won't compile until updated.
fn diff_column_properties(
    ...
    // Exhaustive destructuring ensures we consider all fields.
    // If a new field is added to Column, this code won't compile until updated.
    #[rustfmt::skip]
    let Column {
        name: _,                           // Already matched by caller
        pg_type: desired_pg_type,
        rust_type: _,                      // Metadata only, doesn't affect DB schema
        nullable: desired_nullable,
        default: desired_default,
        primary_key: _,                    // Handled at table level (composite PKs)
        unique: desired_unique,
        auto_generated: desired_auto,
        long: _,                           // UI hint only
        label: _,                          // UI hint only
        enum_variants: _,                  // Derived from type
        doc: _,                            // Documentation only
        icon: _,                           // UI hint only
        lang: _,                           // UI hint only
        subtype: _,                        // UI hint only
    } = desired;
...
mod tests {
    ...
    fn test_diff_alter_column_auto_generated() {
        // Desired: id column has auto_generated = true (user added dibs::auto)
        let mut desired_col = make_column("id", PgType::BigInt, false);
        desired_col.primary_key = true;
        desired_col.auto_generated = true;

        // Current: id column has auto_generated = false (no sequence in DB)
