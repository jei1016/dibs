...
export function getTableLabel(
    ...
    return getTableConfig(config, tableName)?.label ?? tableName;
}
...
export function isFieldReadOnly(
    colName: string,
    detailConfig: DetailConfig | undefined
): boolean {
 ...
 */
export function shouldShowRelations(
    detailConfig: DetailConfig | undefined
): boolean {
